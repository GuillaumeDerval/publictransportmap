...
def extract_travel(in_path, out_path):
    ...
    {
            cities: [("municipality_fr","municipality_nl"),...],
            travel: [
                {"residence" : ("municipality_residence_fr","municipality_residence_nl")
                "work" : ("municipality_work_fr","municipality_work_nl")
                "n" : 0
                },...
    ...
    with open(in_path, newline='') as csvfile:
        ...
        idx = 0
        for row in reader:
            resid = (clean_parenthesis(row["TX_MUNTY_RESIDENCE_DESCR_FR"]),clean_parenthesis(row["TX_MUNTY_RESIDENCE_DESCR_NL"]))
            work = (clean_parenthesis(row["TX_MUNTY_WORK_DESCR_FR"]),clean_parenthesis(row["TX_MUNTY_WORK_DESCR_NL"]))
            n = row["OBS_VALUE"]
